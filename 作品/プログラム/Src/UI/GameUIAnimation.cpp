...
{
	float alpha = MyMath::GetSin(0.0f, 255.0f, rad_);
	ui_->SetAlpha(alpha);
	rad_ += speed_;
}
...
void GameUISlidingAnimation::Step()
{
	...
	VECTOR offset = MyMath::VecScale(move_, scale);
	VECTOR pos = MyMath::VecAdd(start_pos_, offset);
...
// 明滅アニメーション再生
void GameUIAnimationManager::PlayBlinkAnimation(GameUI* ui, float speed)
{
...
// 浮遊アニメーション再生
void GameUIAnimationManager::PlayFloatingAnimation(GameUI* ui, float speed, float floating_size)
{
...
// スライディングアニメーション再生
void GameUIAnimationManager::PlaySlidingAnimation(GameUI* ui, VECTOR start_pos, VECTOR end_pos, float speed)
{
	GameUISlidingAnimation* anim = new GameUISlidingAnimation;
	anim->SetUI(ui);
	anim->SetSpeed(speed);
	VECTOR move = MyMath::VecCreate(start_pos, end_pos);
	anim->SetMove(move);
